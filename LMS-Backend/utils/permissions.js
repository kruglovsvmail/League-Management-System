...
export const ROLES = {                                                          // Экспортируем объект ROLES, где ключи используются в коде, а значения - в базе данных
  ...
  LEAGUE_OWNER: 'league_owner',                                                 // Владелец лиги (league_owners): внутри своей лиги может всё, включая обход дедлайнов и статусов. Штатной ролью не является, в league_staff не хранится
  ...
  GAME_TIMEKEEPER: 'game_timekeeper',                                           // Судья времени матча (маппится с timekeeper)
  GAME_INFORMANT: 'game_informant',                                             // Судья информатор на арене (маппится с informant)
...
export const PERMISSIONS = {                                                    // Экспортируем основной объект PERMISSIONS, связывающий действия с ролями
  ...
  SETTINGS_RESERVE_GOALIES_VIEW: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],       // Право на просмотр списка резервных вратарей дивизиона и их статистики
  SETTINGS_RESERVE_GOALIES_MANAGE: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],     // Право на добавление, изменение и удаление резервных вратарей в списке дивизиона
  ...
  // --------------------------------------------------------------------------
  // РАЗДЕЛ: СДК (Спортивно-дисциплинарный комитет, режим лиги "sdk")
  // --------------------------------------------------------------------------
  SDK_REFERENCES_VIEW: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],                // Право на просмотр справочников СДК (места, участники комиссии, нарушения)
  SDK_REFERENCES_MANAGE: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],              // Право на создание пунктов справочников СДК и удаление мест/участников комиссии
  SDK_VIOLATION_TYPES_DELETE: [ROLES.TOP_MANAGER],                             // Право на удаление пункта из справочника нарушений (только руководитель лиги)
  SDK_MEETINGS_VIEW: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],                  // Право на просмотр заседаний СДК и их состава (явка, представители, документы, решения)
  SDK_MEETINGS_MANAGE: [ROLES.TOP_MANAGER, ROLES.LEAGUE_ADMIN],                // Право на создание/редактирование заседаний, вынесение решений, явку, представителей и документы
  ...
  LEAGUE_GLOBAL_PARAMS_MANAGE: [],                                              // Пустой массив: глобальные параметры лиги (Команды → Лиги) переключает только глобальный администратор
