  ...
  useEffect(() => {
    if (isOpen) {
      if (type === 'base') {
        ...
        let filtered = base || [];
        // При добавлении в ростер — исключаем тех, кто уже в игровом составе
        if (type === 'roster') {
          const rosterUserIds = new Set(roster.map(r => r.user_id));
          filtered = filtered.filter(u => !rosterUserIds.has(u.user_id));
        }
        if (query) {
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar flex flex-col gap-2">
              ...
              {type !== 'base' && users.length === 0 && !query && (
                <div className="text-center text-graphite-light mt-10">
                  Все пользовтели в игровом составе
                </div>
              )}
            </div>
          </>
