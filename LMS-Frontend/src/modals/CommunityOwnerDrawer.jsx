...
/**
 * Назначение владельца сообщества (communities.owner_id) глобальным админом.
 * Поиск глобальный: владелец не обязан состоять в участниках сообщества.
 * Владелец у сообщества один, поэтому выбор нового заменяет прежнего.
 *
 * В отличие от клуба, владелец сообщества получает все права сразу: профиль,
 * настройки, штаб, участники, события и удаление — отдельной роли ему не нужно.
 */
export function CommunityOwnerDrawer({ isOpen, onClose, communityId, communityName, owner, staff = [], onSaved }) {
  ...
  const save = async (userId) => {
    ...
    setError('');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/communities-manage/${communityId}/owner`, {
        method: 'PUT',
  ...
  // Выбранный уже стоит в штабе — предупреждаем, что должность закроется:
  // владелец строкой в штабе не дублируется
  const selectedStaffRow = selectedUser
    ? staff.find(s => s.role !== 'community_owner' && String(s.user_id) === String(selectedUser.id))
    : null;
      ...
      <div className={`absolute top-0 right-0 h-full w-[450px] bg-[#F8F9FA] transform transition-transform duration-300 flex flex-col shadow-2xl ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-graphite/10 bg-white shrink-0">
          <div className="min-w-0">
            <h2 className="font-black text-xl text-graphite uppercase tracking-wide">Владелец сообщества</h2>
            <span className="block text-[12px] text-graphite-light font-bold truncate mt-0.5">{communityName}</span>
          </div>
        ...
        <div className="p-6 bg-white border-b border-graphite/10 shrink-0">
          ...
          {/* Без владельца сообщество никто не может ни настроить, ни удалить —
              напоминаем об этом в момент, когда админ собирается его снять */}
          {isConfirmingRemove && (
            <p className="text-[11px] font-bold text-status-rejected leading-tight mt-3">
              Без владельца в сообществе некому править профиль, назначать штаб и удалять его — эти права есть только у владельца.
            </p>
          )}
        </div>
        ...
        {!selectedUser ? (
          <>
            <div className="p-6 bg-white border-b border-graphite/5 shrink-0 mt-4">
              ...
              <p className="text-[11px] text-graphite-light mt-2 px-1">Глобальный поиск (минимум 3 символа). Владелец не обязан состоять в участниках сообщества.</p>
            </div>
            ...
            <div className="p-6 flex-1 bg-gray-50/50 overflow-y-auto custom-scrollbar">
              {isSameAsCurrent ? (
                <div className="p-4 bg-status-rejected/10 border border-status-rejected/30 rounded-md">
                  <p className="text-[13px] font-bold text-status-rejected leading-tight">Этот пользователь уже является владельцем сообщества.</p>
                </div>
              ) : (
                <div className="p-4 bg-orange/10 border border-orange/30 rounded-md">
                  <p className="text-[13px] font-bold text-orange leading-tight">
                    Пользователь станет владельцем сообщества в Team-Room и получит все права: профиль, настройки, штаб, участники, события и удаление.
                    {owner && ' Прежний владелец это звание потеряет: владелец у сообщества может быть только один.'}
                  </p>
                  {selectedStaffRow && (
                    <p className="text-[12px] font-bold text-graphite-light leading-tight mt-2">
                      Сейчас он в штабе как «{selectedStaffRow.title || (selectedStaffRow.role === 'community_manager' ? 'Руководитель' : 'Администратор')}» — эта должность
                      закроется: владелец в штабе отдельной строкой не дублируется.
                    </p>
                  )}
                </div>
              )}
