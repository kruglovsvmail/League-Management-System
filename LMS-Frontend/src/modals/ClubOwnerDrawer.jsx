...
/**
 * Назначение владельца клуба (clubs.owner_id) глобальным админом.
 * Поиск глобальный: владелец не обязан состоять в общей базе клуба.
 * Владелец у клуба один, поэтому выбор нового заменяет прежнего.
 */
export function ClubOwnerDrawer({ isOpen, onClose, clubId, clubName, owner, onSaved }) {
  ...
  const save = async (userId) => {
    ...
    setError('');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/clubs-manage/${clubId}/owner`, {
        method: 'PUT',
      ...
      <div className={`absolute top-0 right-0 h-full w-[450px] bg-[#F8F9FA] transform transition-transform duration-300 flex flex-col shadow-2xl ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-graphite/10 bg-white shrink-0">
          <div className="min-w-0">
            <h2 className="font-black text-xl text-graphite uppercase tracking-wide">Владелец клуба</h2>
        ...
        {!selectedUser ? (
          <>
            <div className="p-6 bg-white border-b border-graphite/5 shrink-0 mt-4">
              ...
              <p className="text-[11px] text-graphite-light mt-2 px-1">Глобальный поиск (минимум 3 символа). Владелец не обязан состоять в базе клуба.</p>
            </div>
            ...
            <div className="p-6 flex-1 bg-gray-50/50 overflow-y-auto custom-scrollbar">
              {isSameAsCurrent ? (
                <div className="p-4 bg-status-rejected/10 border border-status-rejected/30 rounded-md">
                  <p className="text-[13px] font-bold text-status-rejected leading-tight">Этот пользователь уже является владельцем клуба.</p>
                </div>
              ) : (
                <div className="p-4 bg-orange/10 border border-orange/30 rounded-md">
                  <p className="text-[13px] font-bold text-orange leading-tight">
                    Пользователь станет владельцем клуба в Team-Room.
                    {owner && ' Прежний владелец это звание потеряет: владелец у клуба может быть только один.'}
                  </p>
                  <p className="text-[12px] font-bold text-graphite-light leading-tight mt-2">
                    Права руководителя клуба владелец сам по себе не получает — для них назначьте
                    ему роль «Руководитель клуба» во вкладке «Штаб».
                  </p>
                </div>
              )}
            </div>
