...
export function EditGameInfoDrawer({ isOpen, onClose, game, onSuccess, readOnly = false }) {
  ...
  const homeJerseyImg = currentHomeType === 'dark' 
    ? (game?.home_jersey_dark_url || '/default/jersey_dark.webp') 
    : (game?.home_jersey_light_url || '/default/jersey_light.webp');

  const awayJerseyImg = currentAwayType === 'dark' 
    ? (game?.away_jersey_dark_url || '/default/jersey_dark.webp') 
    : (game?.away_jersey_light_url || '/default/jersey_light.webp');

  return (
    <>
      {/* Фон-затемнение (Backdrop) с эффектом размытия */}
      <div 
