...
export default function BumperOverlay({ game, overlay, sources }) {
  ...
  useEffect(() => {
    ...
    const runTransition = (fromMs = 0, onStart) => {
      ...
      const begin = () => {
        // Всё расписание — отметки COVER_MS и SWEEP_MS на дорожке файла — верно,
        // только если ноль файла и есть ноль рисунка. Файл, собранный панелью до
        // исправления сборщика, длиннее сценария на задержку старта рекордера:
        // на столько же в нём опаздывают шторки, и ролик вылезает раньше них.
        // Лечится только пересборкой перехода в панели — отсюда и предупреждение.
        const fileMs = t.duration * 1000;
        if (Number.isFinite(fileMs) && (fileMs < SWEEP_MS - 50 || fileMs > SWEEP_MS + 300)) {
          console.warn(`[Заставка] длительность перехода ${Math.round(fileMs)} мс не сходится со сценарием ${SWEEP_MS} мс — файл собран старой версией панели, пересоберите переход`);
        }
        start();
        onStart?.();
      };
