...
export function PlayoffStructureView({ brackets = [] }) {
    ...
    // Умная подготовка данных: фильтрация основного матча раунда и вынос последующих серий вниз
    const roundsData = useMemo(() => {
        ...
        return sorted.map(round => {
            ...
            // Проверяем, есть ли в текущем раунде обычные серии (без привязки к конкретному месту)
            const hasRegularMatchups = matchups.some(m => {
                const type = m.ui_metadata?.match_type;
                return !type || type === 'regular';
            });

            let mainMatchups = [];
            let consolationMatchups = [];

            if (hasRegularMatchups) {
                // Если есть стандартные серии (например, 1/4 или 1/2 финала), они все идут в основную сетку.
                // Если вдруг админ добавил в этот же раунд стыковые матчи за места — они идут вниз.
                matchups.forEach(m => {
                    const type = m.ui_metadata?.match_type;
                    if (!type || type === 'regular') {
                        mainMatchups.push(m);
                    } else {
                        consolationMatchups.push(m);
                    }
                });
            } else {
                // Если стандартных серий нет (это раунд финалов, например «Финал» главной или утешительной сетки)
                // Находим ОДНУ серию с наивысшим местом (минимальное число в названии, например place_5 выше, чем place_7)
                let highestPlaceMatchup = null;
                let minPlaceNumber = Infinity;

                matchups.forEach(m => {
                    const type = m.ui_metadata?.match_type || '';
                    const match = type.match(/^place_(\d+)$/);
                    if (match) {
                        const placeNum = parseInt(match[1], 10);
                        if (placeNum < minPlaceNumber) {
                            minPlaceNumber = placeNum;
                            highestPlaceMatchup = m;
                        }
                    }
                });

                // Распределяем: лучшую серию — в дерево, все последующие (низшие места) — в распределение мест вниз
                matchups.forEach(m => {
                    if (highestPlaceMatchup && m.id === highestPlaceMatchup.id) {
                        mainMatchups.push(m);
                    } else {
                        consolationMatchups.push(m);
                    }
    ...
    // Вычисляем флаг компактности на основе общего количества раундов
    const isCompact = roundsData.length >= 4;
    ...
    return (
        <div className="flex flex-col gap-6 w-full">
            ...
            <div className="relative w-full overflow-x-auto custom-scrollbar pb-8 pt-4">
                <div className={`flex min-h-[450px] w-max px-4 ${isCompact ? 'gap-4' : 'gap-12'}`}>
                    {roundsData.map((round, rIndex) => (
                        <div key={round.id} className={`flex flex-col shrink-0 relative ${isCompact ? 'w-[220px]' : 'w-[280px]'}`}>
                                ...
                                {round.mainMatchups.map(matchup => {
                                    ...
                                    return (
                                        <div key={matchup.id} className="relative flex w-full">
                                            ...
                                            {/* Визуальные соединительные линии (Ветки дерева) с адаптивным отступом и шириной */}
                                            {rIndex > 0 && (
                                                <div className={`absolute top-1/2 h-[2px] bg-graphite/15 -z-10 ${
                                                    isCompact ? '-left-[8px] w-[8px]' : '-left-[24px] w-[24px]'
                                                }`} />
                                            )}
                                            {rIndex < roundsData.length - 1 && (
                                                <div className={`absolute top-1/2 h-[2px] bg-graphite/15 -z-10 ${
                                                    isCompact ? '-right-[8px] w-[8px]' : '-right-[24px] w-[24px]'
                                                }`} />
                                            )}
                    ...
                    <div className="flex flex-wrap gap-6">
                        {roundsData.map(round => (
                            round.consolationMatchups.map(matchup => {
                                ...
                                return (
                                    <div key={matchup.id} className="flex flex-col w-[230px] bg-graphite/5 border border-graphite/10 rounded-2xl p-3 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex items-center justify-between mb-3 px-1">
