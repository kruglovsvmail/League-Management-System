...
const MatchupSettingsModal = ({ data, activeBracketId, round, matchup, onSave, onClose }) => {
    ...
    const isFinalRound = round.name === 'Финал';
    const [matchType, setMatchType] = useState(matchup.match_type === 'regular' && isFinalRound ? 'place_1' : matchup.match_type || 'regular');
    ...
    const MATCH_TYPE_VALUES = ['place_1', 'place_3', 'place_5', 'place_7', 'place_9', 'place_11', 'place_13', 'place_15'];

    const handleLocalSave = () => {
        const finalMatchType = isFinalRound ? matchType : 'regular';

        if (isFinalRound) {
            const duplicate = round.matchups.find(m => m.id !== matchup.id && m.match_type === finalMatchType);
            if (duplicate) {
                const placeLabel = MATCH_TYPE_LABELS[MATCH_TYPE_VALUES.indexOf(finalMatchType)];
                setModalError(`Ошибка: Розыгрыш за "${placeLabel}" уже назначен в паре №${duplicate.num}. Каждое место должно быть уникальным.`);
                return;
            }
        }

        onSave(s1Type, s1Ref, s2Type, s2Ref, finalMatchType);
    };
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title={`Настройки пары (Р${round.order}П${matchup.num})`} size="wide">
            <div className="flex flex-col gap-4 font-sans">
                {isFinalRound && (
                    <div className="p-5 bg-white/50 border border-graphite/10 rounded-2xl flex flex-col gap-4">
                        ...
                        <SegmentButton 
                            ...
                            onChange={(idx) => {
                                setMatchType(MATCH_TYPE_VALUES[idx]);
                                setModalError('');
                            }}
                        />
                    </div>
                ...
                {modalError && (
                    <div className="p-3 bg-status-rejected/10 text-status-rejected border border-status-rejected/20 rounded-xl text-[13px] font-medium animate-zoom-in">
                        {modalError}
                    </div>
                )}

                <div className="flex justify-end pt-2">
                    <Button onClick={handleLocalSave} className="w-full md:w-auto px-10">
                        Сохранить изменения
                    </Button>
                </div>
            </div>
...
export function PlayoffConstructor({ divisionId: propDivisionId }) {
    ...
    const handleMouseMove = (e) => {
        ...
        if (nodeDragging) {
            if (nodeDragging.type === 'round') {
                ...
                const dy = (e.clientY - nodeDragging.startY) / zoom;
                const proposedY = Math.max(0, nodeDragging.initY + dy);
                const currentRound = activeBracket.rounds.find(r => r.id === nodeDragging.initRoundId);
                ...
                if (canMoveY && proposedY !== movingMatchup.y) {
                    updateActiveBracket(b => ({
                        ...b, rounds: b.rounds.map(r => r.id === nodeDragging.initRoundId ? {
    ...
    const handleMatchupDragStart = (e, roundId, matchup) => {
        ...
        e.preventDefault(); e.stopPropagation();
        setNodeDragging({ type: 'matchup', initRoundId: roundId, matchupId: matchup.id, startY: e.clientY, initY: matchup.y });
    };
    ...
    const addRound = () => {
        ...
        updateActiveBracket(b => ({ ...b, rounds: [...b.rounds, { id: genId(), order: b.rounds.length + 1, name: '1/8 финала', wins: 2, x: maxX + 300, y: 50, matchups: [] }] }));
    };
    ...
    return (
        ...
        >
                                                ...
                                                <div className="flex flex-col gap-2 relative z-10">
                                                    {[1, 2].map(slotNum => {
                                                        ...
                                                        if (type === 'seed') {
                                                            ...
                                                            const suffix = `${isWin ? 'Поб.' : 'Проигр.'} (Р${refMatch.roundOrder}П${refMatch.num})`;
            ...
            {editRoundParams && (() => {
                const presets = ['1/8 финала', '1/4 финала', '1/2 финала', 'Финал'];
                const currentIdx = presets.indexOf(editRoundParams.name);
                    ...
                    <Modal isOpen={true} onClose={() => { setEditRoundParams(null); setRoundModalError(''); }} title="Настройки раунда" size="normal">
                        <div className="flex flex-col gap-6 font-sans">
                            ...
                            <div className="flex flex-col gap-2">
                                <span className="text-[12px] font-bold text-graphite uppercase tracking-wide">Раунд плей-офф</span>
                                <SegmentButton 
                                    options={presets}
                                    defaultIndex={currentIdx !== -1 ? currentIdx : 0}
                                    onChange={(idx) => {
                                        setEditRoundParams({ ...editRoundParams, name: presets[idx] });
                                        setRoundModalError('');
                                    }}
                                />
                            </div>
                            ...
                            <div className="flex flex-col gap-3 pt-4 border-t border-graphite/10">
                                <Button 
                                    onClick={() => { 
                                        if (!editRoundParams.name.trim()) {
                                            setRoundModalError('Название раунда не может быть пустым.');
                                            return;
                                        }
                                        ...
                                        // Если переименовываем раунд ИЗ Финала во что-то другое, очищаем match_type во всех парах
                                        const isChangingFromFinal = editRoundParams.originalName === 'Финал' && editRoundParams.name !== 'Финал';
                                        ...
                                        updateActiveBracket(b => ({
                                            ...
                                            rounds: b.rounds.map(r => r.id === editRoundParams.id 
                                                ? {
                                                    ...r, 
                                                    name: editRoundParams.name, 
                                                    wins: editRoundParams.wins,
                                                    matchups: isChangingFromFinal 
                                                        ? r.matchups.map(m => ({ ...m, match_type: 'regular' }))
                                                        : r.matchups
                                                  } 
                                                : r)
                                        })); 
