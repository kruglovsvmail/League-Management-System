...
const ShootoutColumn = ({ isPending, isClosed, teamId, teamLetter, teamName, teamLogo, roster, oppRoster, events, onSaveEvent, onDeleteEvent, soLength, periodLength, otLength, periodsCount, isReadOnly }) => {
  ...
  return (
    <div className={`flex-1 min-w-0 bg-white border border-graphite/20 shadow-sm rounded-md relative transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isPending ? 'opacity-30 grayscale blur-[1px] pointer-events-none scale-[0.99] select-none' : 'opacity-100 scale-100'}`}>
       <div className="font-bold text-graphite text-base uppercase tracking-wide flex items-center gap-3 px-5 py-3 bg-white rounded-md">
                    ...
                    return (
                        <tr key={i} className="even:bg-graphite/[0.02] hover:bg-graphite/5 transition-colors group h-[36px] border-b border-graphite/30">
                            ...
                            {shot ? (
                                ...
                                <>
                                    <td className="p-1 border-r border-graphite/30 text-center">
                                        <StylishSelect title="Бросающий" roster={roster} value={newShot.player} onChange={e=>setNewShot({...newShot, player: e.target.value})} className="font-bold" />
                                    </td>
                                    <td className="p-1 border-r border-graphite/30 text-center">
                                        <div className="flex items-center justify-center gap-1">
                                            {goalies.map(g => (
...
export const ShootoutAccordion = ({ 
  ...
  return (
    <div className={`bg-white shadow-lg flex flex-col font-sans rounded-md transition-all duration-500 ease-in-out ${currentPeriod === 'SO' ? 'opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}>
