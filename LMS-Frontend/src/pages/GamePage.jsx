...
export function GamePage() {
  ...
  const getJerseyUrl = (teamPrefix, type) => {
    ...
    // Приоритет: tournament_teams.custom_jersey → teams.jersey → дефолт
    const url = game[`${teamPrefix}_jersey_${type}_url`] || `/default/jersey_${type}.webp`;
        ...
        <div className={`flex items-center gap-2 mb-1.5 w-full ${isHome ? 'justify-start' : 'justify-end'}`}>
          {isGoal && ev.goal_strength !== 'equal' && (
            <span className="text-[9px] font-bold text-orange uppercase bg-orange/10 px-1.5 py-0.5 rounded shadow-sm">
               {['pp1','pp2'].includes(ev.goal_strength) ? 'Большинство' : ['sh1','sh2'].includes(ev.goal_strength) ? 'Меньшинство' : ev.goal_strength === 'en' ? 'В пусты ворота' : ev.goal_strength === 'ps' ? 'Штрафной бросок' : ev.goal_strength}
            </span>
          )}
          {isPenalty && (
             <span className="text-[9px] font-bold text-orange uppercase bg-orange/10 px-1.5 py-0.5 rounded shadow-sm">
