    ...
    try {
      ...
      if (avatarFile) {
        ...
        if (uploadData.success) {
          finalAvatarUrl = `uploads/${customFileName}`;
        }
      }
  ...
  return (
    <div className={`fixed inset-0 z-[35] transition-opacity duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <div className={`absolute top-0 left-[230px] h-full w-[400px] max-w-[calc(100vw-230px)] bg-white/85 transform transition-transform duration-300 flex flex-col shadow-[24px_0_24px_rgba(0,0,0,0.1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {/* Шапка шторки: теперь вся область кликабельна */}
        <div 
          onClick={onClose} 
          className="flex items-center p-5 border-b border-graphite/10 cursor-pointer hover:bg-gray-50 transition-colors group"
        >
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-7">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="w-[140px] [&_.gap-4]:!hidden [&_.rounded-xl]:!rounded-xxl [&_.rounded-xl]:!bg-graphite/5">
              <Uploader 
                heightClass="h-[140px]"
                initialUrl={user?.avatarUrl ? getImageUrl(user.avatarUrl) : null}
          ...
          <div className="space-y-4">
            <h4 className="text-[11px] font-bold text-graphite-light uppercase tracking-wider mb-3">
            ...
            <div>
              ...
              {!isEditingPhone ? (
                <div className="relative flex items-center w-full border border-graphite/30 rounded-md bg-gray-50 transition-all duration-300 group hover:border-graphite/50">
                  ...
                  <input 
                    ...
                    disabled
                    className="w-full px-3 py-2.5 bg-transparent text-graphite/70 text-[13px] font-medium outline-none cursor-not-allowed"
                  />
                  <button onClick={() => setIsEditingPhone(true)} className="absolute right-3 text-graphite-light hover:text-orange transition-colors opacity-0 group-hover:opacity-100 bg-gray-50 pl-2">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                  </button>
                </div>
              ) : (
                <div className="relative flex items-center w-full border border-graphite/40 rounded-md bg-white transition-all duration-300 focus-within:border-orange focus-within:shadow-[0_0_0_3px_rgba(255,122,0,0.2)]">
                  ...
                  <input 
                    ...
                    className="w-full px-3 py-2.5 bg-transparent text-graphite text-[13px] font-medium outline-none"
                    autoFocus
                  />
                </div>
              )}
              ...
              {!isEditingEmail ? (
                <div className="relative flex items-center w-full border border-graphite/30 rounded-md bg-gray-50 transition-all duration-300 group hover:border-graphite/50">
                  <input 
                    ...
                    className="w-full px-3 py-2.5 bg-transparent text-graphite/70 text-[13px] font-medium outline-none cursor-not-allowed"
                  />
                  <button onClick={() => setIsEditingEmail(true)} className="absolute right-3 text-graphite-light hover:text-orange transition-colors opacity-0 group-hover:opacity-100 bg-gray-50 pl-2">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                  </button>
                </div>
              ) : (
          ...
          <div className="space-y-4 pb-4">
            ...
            {!isEditingPassword ? (
              <button 
                ...
                className="w-full flex items-center justify-between px-3 py-2.5 bg-gray-50 border border-graphite/20 rounded-md hover:border-graphite/40 transition-colors group"
              >
                <span className="text-[13px] font-medium text-graphite">Сменить пароль</span>
                <svg className="text-graphite-light group-hover:text-orange transition-colors" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
              </button>
            ) : (
              <div className="animate-fade-in-down">
                <Input value={password} onChange={e => setPassword(e.target.value)} placeholder="Введите новый пароль" />
              </div>
            )}
          </div>
