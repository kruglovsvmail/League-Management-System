...
export function CreateDisqualificationModal({ isOpen, onClose, divisions = [], onSuccess }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Назначение дисквалификации" size="wide">
      <div className="flex flex-col md:flex-row gap-6">
        
        {/* Левая панель - Форма */}
        <div className="w-full md:w-[320px] shrink-0 flex flex-col gap-6 md:border-r border-graphite/10 md:pr-6">
        ...
        {/* Правая панель - Выбор игрока */}
        <div className="flex-1 flex flex-col gap-4">
          <Input placeholder="Поиск игрока по ФИО..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          <div className="flex-1 min-h-[450px] max-h-[450px] overflow-y-auto custom-scrollbar flex flex-col gap-2 pr-2">
            {isLoadingPlayers ? (
              ...
              filteredPlayers.map(p => (
                ...
                >
                  <div className="w-[42px] h-[42px] rounded-lg bg-graphite/10 overflow-hidden shrink-0 flex items-center justify-center">
                    <img src={getImageUrl(p.team_member_photo_url || p.user_avatar_url || '/default/user_default.webp')} alt="avatar" className="w-full h-full object-cover" />
                  </div>
          ...
          <div className="mt-2 pt-4 border-t border-graphite/10 shrink-0">
            {/* ИЗМЕНЕНИЕ: Динамический className. Серая, если заблокирована, Красная - если активна */}
            <Button 
              ...
              className={
                isFormValid && !isSubmitting 
                  ? "w-full bg-status-rejected text-white border-none hover:brightness-90 transition-all" 
                  : "w-full"
              }
            >
