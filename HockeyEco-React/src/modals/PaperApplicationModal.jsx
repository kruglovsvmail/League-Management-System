...
export function PaperApplicationModal({ isOpen, onClose, app, onSave, isSaving, readOnly = false }) {
  ...
  return (
    ...
    >
      ...
      {readOnly && (
        <AccessFallback variant="readonly" message="Режим просмотра. Скачивание и загрузка заявок недоступны." />
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-[30px] mb-6 items-start mt-2">
        <div className="flex flex-col w-full">
          ...
          {app?.paper_roster_team_url ? (
            <div className={`h-[232px] w-full border-2 border-dashed border-graphite/20 rounded-2xl flex flex-col items-center justify-center bg-white gap-4 group transition-colors ${readOnly ? 'opacity-70' : 'hover:border-orange'}`}>
              <div className="w-16 h-16 rounded-full bg-orange/10 flex items-center justify-center text-orange transition-transform group-hover:scale-110">
              ...
              {/* Ссылку показываем только если НЕ readOnly */}
              {!readOnly ? (
                <a 
                ...
                <span className="font-bold text-[15px] text-graphite/50">Файл загружен</span>
              )}
            </div>
          ) : (
      ...
      {!readOnly && (
        <div className="flex justify-end pt-5 border-t border-graphite/10">
          <Button 
            ...
            className="bg-orange text-white border-none transition-all duration-300 hover:bg-orange-hover"
          >
            Сохранить
          </Button>
        </div>
